    ...
    <div className="d-flex ">
      ...
      <div className="Profile-userCard">
        <div className="d-flex align-items-center">
          <div className="">
            {" "}
            <img src={user.imageUrl} className="Profile-image" />{" "}
          </div>
          <div className="ml-3 w-100 Profile-content">
            <h4 className="mb-0 mt-0 dep">
              {user.firstname} {user.lastname}
              {/* {isCurrentUser && <label className="u-link" onClick={open} >
                   <tab className="Profile-tab"> Edit </tab>        
                  </label>} */}
            </h4>{" "}
            <span className="dep">{user.description}</span>
            <div className="here d-flex justify-content-center rounded text-white stats dep">
              <div className="d-flex flex-column">
                {" "}
                <span className="articles">Museums</span>{" "}
                <span className="number1">{museumCount}</span>{" "}
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="Profile-user">
        {userProfile && (
          <UserProfile
            ...
            museumCount={userMuseums.length}
            editUserFunction={editUserFunction}
          />
        )}
      </div>
      {userProfile && (
        <div className="Profile-museums cards">
          {userMuseums.length > 0 ? (
            ...
            <h4 className="mb-0 mt-0 Profile-tab">
              {" "}
              No {isCurrentUser && "public"} museums to show.{" "}
            </h4>
          )}
        </div>
      )}
